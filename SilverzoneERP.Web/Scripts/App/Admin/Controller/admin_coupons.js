(function () {
    ...
    coupons_list_ControllerFn.$inject = ['$scope', '$rootScope', '$filter', 'admin_bookCategory_Service', '$uibModal', 'admin_modalService']
    function coupons_list_ControllerFn($sc, $rsc, $filter, svc, $modal, modalSvc) {
        ...
        function showModal(_model) {
            var Template =' <div class="modal-header" style="padding: 10px !important;">                                                                                             '
                         ...
                         + ' <input type="text" class="form-control1" placeholder="Coupon Name" ng-model="coupon.Coupon_name" name="Coupon_name">                                     '
                         + ' </div><div class="col-sm-6">                                                                                                                            '
                         + ' <input type="text" class="form-control1" placeholder="Coupon Amount" ng-model="coupon.Coupon_amount" name="Coupon_amount" number-only>                   '
                         + ' </div> </div> <div class="form-group">                                                                                                                  '
                         + ' <div class="col-sm-6">                                                                                                                                  '
                         + ' <textarea rows="3" class="form-control1 ng-empty" placeholder="Coupon Description" ng-model="coupon.Description" name="Description"></textarea>          '
                         ...
                         + ' <input type="text" class="form-control1" ng-model="coupon.Start_time" placeholder="Start Date" name="Start_time">                                        '
                         ...
                         + ' <input type="text" class="form-control1" ng-model="coupon.End_time" placeholder="End Date" name="End_time">                                              '
        ...
        $sc.Show_deleteModal = function (Id) {
            ...
            modalSvc.deleteModal('Coupon').then(function (data) {
                ...
                svc.delete_coupon(couponInfo)
                   .then(function () {
                       $rsc.notify_fx('Coupon has been deleted successfully !', 'info');
                   })
            });
        }
